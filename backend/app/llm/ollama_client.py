...
class OllamaClient:
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Timeout checking Ollama health: {str(e)}"
            )
    ...
    def generate(
        ...
        logger.info(
            f"Sending chat request to Ollama model "
            f"'{self.model}' (messages={len(messages)})"
        )
        ...
        try:
            with httpx.Client(timeout=self.timeout_sec) as client:
                ...
                if isinstance(message_obj, dict):
                    answer = message_obj.get(
                        "content",
                        "",
                    ).strip()

                logger.info(
                    f"Received response from Ollama in "
                    f"{latency}s "
                    f"(tokens={result.get('eval_count', 'N/A')})"
                )
            ...
            logger.error(
                f"Ollama connection error during generate: {str(e)}"
            )
            raise OllamaConnectionError(
            ...
            logger.error(
                f"Ollama generate request timed out "
                f"(> {self.timeout_sec}s): {str(e)}"
            )
            raise OllamaTimeoutError(
                f"Ollama inference timed out after "
